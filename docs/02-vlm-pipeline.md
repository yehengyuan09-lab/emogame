...
| 层级 | 模型 | 部署方式 | 用途 | 延迟 | 成本 |
|------|------|----------|------|------|------|
| L1 快速提取 | InternVL2-4B | 本地 GPU (T4, 16GB) | 批量皮肤分类、色彩提取、构图分析 | ~0.5s/img | 免费 |
| L2 精细分析 | Qwen2-VL-7B | 本地 GPU (A10, 24GB) | 审美维度评分、特效层级判别、缺陷检测 | ~2s/img | 免费 |
| L3 深度理解 | GPT-4o-mini | API 调用 | 设计语言解读、文化符号识别、竞品对比描述 | ~3s/img | ~$0.00015/img |

### 模型能力对比

#### InternVL2-4B（L1 — 批量快速分类）

- **优势**：开源可本地部署，中文图文训练集（MMBench/MMStar 基准领先），4B 参数可在 T4 上运行
- **适用任务**：颜色直方图、构图密度、场景分类、UI 标签检测
- **局限**：对高分辨率细节（1920×882 壁纸）理解有限

#### Qwen2-VL-7B（L2 — 精细审美分析）

- **优势**：原生中文 OCR，游戏 UI 元素识别准确，支持 4480×4480 高分辨率输入
- **适用任务**：面部细节、服饰纹理、特效粒子层数、UI 标识（限定标签、价格标签）
- **局限**：需要 A10 24GB VRAM，不能批量高并发
...
```bash
# 先拉取可用的视觉模型；Qwen2.5-VL 可作为 Qwen2-VL 的 Ollama 近邻替代
ollama pull qwen2.5vl:3b

# 若本机已有 InternVL2 的 Ollama 模型名，可替换/追加到 --models
python3 vlm/ollama_vlm_test.py \
  --models internvl2:4b,qwen2-vl:7b,qwen2.5vl:3b \
  --image hero-skin-image/3phone-bigskin-images/李白-3-千年之狐.jpg \
  --prompt l2
```

脚本会跳过未安装模型并给出 `ollama pull <model>` 提示；已安装模型会输出模型名、耗时和原始 JSON 响应。当前 Ollama 官方库中 Qwen 视觉模型较容易直接拉取，InternVL2 如果来自自建 Modelfile 或第三方 GGUF，只需把实际模型名传给 `--models` 即可复用同一测试链路。
...
# 策略 2: 动态批处理
# InternVL2-4B batch_size=8 on T4 (16GB VRAM)
...
# 策略 4: 降级策略
# GPU 不可用时: L1+L2 降级为 GPT-4o-mini API 模式
...
| 开发 | T4 | 16GB | InternVL2-4B (batch=8) |
| 生产 | A10 | 24GB | InternVL2-4B + Qwen2-VL-7B 并行 |
